...
class Model(pl.LightningModule):
    ...
    def forward(self, x: dict[BatchKey, torch.Tensor]) -> torch.Tensor:
        ...
        new_batch_size = byte_array.shape[0]
        ...
        learnt_encoder_query = einops.repeat(
            self.encoder_query,
            "element feature -> example element feature",
            example=new_batch_size,
        )
        decoder_query_padded_for_encoder = maybe_pad_with_zeros(
            decoder_query, requested_dim=self.encoder_query_dim
        )
        encoder_query = torch.concat(
            (learnt_encoder_query, decoder_query_padded_for_encoder), dim=1
        )
